...
// Класс state реализует булевское состояние, общее для разных потоков.
// Вызов метода wait() усыпляет вызвавший поток до установки состояния.
// Между моментами установки и сброса состояния ожидание истинного состояния
// функцией wait() сразу же завершается для всех вызвавших его потоков.
// Фактически состояние представляет собой событие с ручным сбросом.
// Метод set() устанавливает состояние в соответствии со значением аргумента.
