...
// –азмеры буферов дл€ вывода значений специфических типов
// ¬о всех случа€х указан размер полного буфера, включа€ место под завершающий нуль.
...
{
  alloc(src.len());       // отводим пам€ть только под реальное содержимое
...
{
  int slen = this->len();
  test_index(pos, slen+1); // позвол€ем адресовать завершающий нуль
